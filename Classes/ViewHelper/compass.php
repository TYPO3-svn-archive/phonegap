...
/* This view helper implements Phonegap compass class.
 * 
 *
 * @license http://www.gnu.org/licenses/lgpl.html GNU Lesser General Public License, version 3 or later
 */
class Tx_Phonegap_ViewHelpers_CompassViewHelper extends Tx_Fluid_Core_ViewHelper_AbstractViewHelper implements Tx_Fluid_Core_ViewHelper_Facets_ChildNodeAccessInterface {

	public function __construct() {
		$this->registerArgument('frequency', 'mixed', 'Value to be returned if the condition if met.', FALSE);
	...
	public function render() {
		...
		//$GLOBALS['TSFE']->additionalHeaderData[] = '<meta name="mobile-access" content="geolocation">';
		
		$output='<script>
				if (navigator.compass){
					navigator.compass.watchHeading(
						function (heading){'.$this->renderSuccessChild().';},
						function (){'.$this->renderErrorChild().';},
						{ frequency: '.$this->arguments["frequency"].' }
					);
				}
			</script>
